...
-- 2. Migrar datos: sumar las cantidades servidas de product_order_item a product_order
UPDATE product_order po
    LEFT JOIN (
        SELECT product_order_id, SUM(served_quantity) as total_served
        FROM product_order_item
        GROUP BY product_order_id
    ) poi ON po.id = poi.product_order_id
SET po.served_quantity = COALESCE(poi.total_served, 0);

-- 3. Actualizar el status de product_order basado en served_quantity
...
-- 4. Eliminar la tabla product_order_item
